...
export function buildResidentQuestionStatus(attempts) {
  ...
  for (const a of attempts || []) {
    ...
    if (!status[a.resident_id][a.module_id]) {
      status[a.resident_id][a.module_id] = {
        hasCorrect: false,
        hasWrong: false,
      };
    }
    if (a.outcome === "correct")
      status[a.resident_id][a.module_id].hasCorrect = true;
    if (a.outcome === "incorrect")
      status[a.resident_id][a.module_id].hasWrong = true;
  }
  return status;
}
...
export function getResidentQuestionBreakdown(attempts, dungeon, residentId) {
  ...
  for (const a of attempts || []) {
    if (
      ...
      !dungeon.questionIds.includes(a.module_id)
    )
      continue;
    if (!byQuestion[a.module_id])
      byQuestion[a.module_id] = { correct: 0, wrong: 0 };
...
export function getCohortDungeonMetrics(residentIds, attempts, dungeons) {
  ...
  return dungeons.map((d) => {
    ...
    return {
      ...
      completionPct:
        questionSlots > 0
          ? Math.round((completedCount / questionSlots) * 100)
          : 0,
      wrongPct:
        questionSlots > 0 ? Math.round((wrongCount / questionSlots) * 100) : 0,
    };
  });
}
