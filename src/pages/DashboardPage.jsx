...
const COHORT_COLORS = [
  CHART_COLORS.royalBlue,
  CHART_COLORS.fantasyGreen,
  CHART_COLORS.flagYellow,
  CHART_COLORS.roofRed,
];
...
export function DashboardPage() {
  ...
  useEffect(() => {
    ...
    async function fetchData() {
      ...
      residentCohorts?.forEach((rc) => {
        if (!cohortResidents[rc.cohort_id])
          cohortResidents[rc.cohort_id] = new Set();
      ...
      cohorts?.forEach((c) => {
        ...
        const metrics = getCohortDungeonMetrics(
          residentsInCohort,
          attemptsData ?? [],
          DUNGEONS,
        );
        rows.push({
          ...
          cells: Object.fromEntries(
            metrics.map((m) => [
              m.dungeonId,
              { completionPct: m.completionPct, wrongPct: m.wrongPct },
            ]),
          ),
        });
      });

      const allMetrics = getCohortDungeonMetrics(
        allResidentIds,
        attemptsData ?? [],
        DUNGEONS,
      );
  ...
  const avgCompletion =
    allRow && modules.length > 0
      ? Math.round(
          Object.values(allRow.cells).reduce(
            (a, c) => a + (c?.completionPct ?? 0),
            0,
          ) / modules.length,
        )
      : 0;
    ...
    const list = residents
      ...
      .map((r) => {
        ...
        const pct =
          progress.length > 0
            ? Math.round(
                progress.reduce((a, p) => a + p.completionPct, 0) /
                  progress.length,
              )
            : 0;
        return {
        ...
        <div className="bg-surface-card border-2 border-border-dark rounded-sm p-6">
          <p className="text-text-muted text-sm py-12 text-center">
            ...
            <Link
              ...
              className="text-royal-blue-light hover:underline"
            >
              Residents
            </Link>{" "}
            page.
